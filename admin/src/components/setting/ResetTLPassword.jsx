...
export default function ResetTLPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.put(
        `${BASE_URL}/super-admin/reset-password/tl`,
        { newPassword },
        {
  ...
  const renderInput = (name, label, showKey) => (
    <div>
      <label
        htmlFor={`tl-${name}`}
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">
            Set a new password for the Team Leader account
          </p>
        </div>
      </div>
