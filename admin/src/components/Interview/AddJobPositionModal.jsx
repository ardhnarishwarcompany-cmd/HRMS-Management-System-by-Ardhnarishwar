...
export default function AddJobPositionModal({ open, onClose, onSuccess }) {
  ...
  const handleSubmit = async () => {
    if (!title.trim()) return toast.error("Enter job title");

    try {
      setLoading(true);
      await createJobPosition({ title: title.trim() });
      setTitle("");
      onSuccess?.();
      onClose();
    } catch (err) {
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                New role candidates can be scheduled for
              </p>
            </div>
          </div>
          <button
        ...
        <div className="px-6 py-5">
          <label className="mb-1.5 block text-[13px] font-medium text-gray-600">
            Job Title<span className="ml-0.5 text-red-500">*</span>
          </label>
          <input
            type="text"
